...
async function createAtriaKey(account, options = {}) {
  ...
  try {
    ...
    console.log(`  Navigating to ${CONSOLE_KEYS_URL} (will redirect to sign-in)...`);
    ...
    console.log("  Processing post-password / consent steps...");
    const atriaPage = await completeGoogleLogin(browser, googlePage, 90000);
    console.log(`  Login completed. Current URL: ${atriaPage.url()}`);
    await dismissBubbles(atriaPage);

    console.log(`  Navigating to ${CONSOLE_KEYS_URL}...`);
    ...
    console.log('  Clicking "Create key"...');
    ...
    console.log("  Reading generated API key...");
